...
const UpdateListing = () => {
  ...
  useEffect(() => {
    ...
    const getListing = async() => {
      ...
      if(docSnap.exists()){
        ...
        navigate('/')
        toast.error('Listing not exisit')
      }
    }
    getListing()
  },[params.id, navigate])

  // getting user to logged in user
  useEffect(() => {
  ...
  useEffect(() => {
    if(listing && listing.userRef !== auth.currentUser.uid){
      toast.error('User can not edit the listing')
      navigate('/')
    }
  })
    ...
    <>
      <main className='h-full md:h-screen'>
        ...
        <div className='w-full flex justify-center items-center mt-4'>
          <button className='w-48 bg-green-900 p-2 rounded-xl text-emerald-100 font-semibold'>Update Listing</button>
        </div>
